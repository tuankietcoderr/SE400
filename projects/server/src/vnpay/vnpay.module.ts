...
import type { VnpayModuleAsyncOptions, VnpayModuleOptions, VnpayModuleOptionsFactory } from 'src/common/interfaces';
...
export class VnpayModule {
  static register(options: VnpayModuleOptions): DynamicModule {
    return {
      ...
      exports: [VnpayService, VnpayModule]
    };
  }
  ...
  static registerAsync(options: VnpayModuleAsyncOptions): DynamicModule {
    return {
      ...
      providers: [...VnpayModule.createAsyncProviders(options), ...(options?.extraProviders ?? []), VnpayService],
  ...
  private static createAsyncOptionsProvider(options: VnpayModuleAsyncOptions): Provider {
    ...
    return {
      provide: VNPAY_MODULE_OPTIONS,
      useFactory: async (optionsFactory: VnpayModuleOptionsFactory) => optionsFactory.createVnpayOptions(),
